...
namespace Com.Lmax.Api.Internal
{
    public class HttpInvoker : IHttpInvoker
    {
        ...
        public HttpInvoker() : this(new LmaxApiOptions())
        {
        }

        public HttpInvoker(LmaxApiOptions apiOptions)
        {
            _userAgent = "LMAX .Net API, version: " + Assembly.GetExecutingAssembly().GetName().Version + ", id: " +
                         apiOptions.ClientIdentifier;
            ServicePointManager.DefaultConnectionLimit = apiOptions.DefaultConnectionLimit;
        }
        ...
        private Exception translateException(WebException e)
        {
                ...
                {
                    return new UnexpectedHttpStatusCodeException(response.StatusCode);
                }
        }
